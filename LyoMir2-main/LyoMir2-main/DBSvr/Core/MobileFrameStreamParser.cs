...
{
    /// <summary>
    /// Preserves incomplete 44FF44FF frames between socket receive callbacks.
    /// </summary>
    public sealed class MobileFrameStreamParser
    {
        ...
        private byte[] _buffer = new byte[InitialBufferLength];
        private int _length;

        public MobileFrameStreamParser(int maximumBufferedLength = DefaultMaximumBufferedLength)
        {
            ...
            if (_buffer.Length > maximumBufferedLength)
                Array.Resize(ref _buffer, maximumBufferedLength);
        }
            ...
            out List<Frame44FF44> frames, out string error)
        {
            ...
            if (_length + count > _maximumBufferedLength)
                return Fail($"44FF44FF stream exceeds {_maximumBufferedLength} buffered bytes", out error);
            ...
            if (consumed > 0)
            {
                ...
                if (_length == 0 && _buffer.Length > InitialBufferLength)
                    Array.Resize(ref _buffer, Math.Min(InitialBufferLength, _maximumBufferedLength));
            }
            return true;
        }
        ...
        private void EnsureCapacity(int required)
        {
            ...
            var size = Math.Min(_maximumBufferedLength, Math.Max(required, _buffer.Length * 2));
