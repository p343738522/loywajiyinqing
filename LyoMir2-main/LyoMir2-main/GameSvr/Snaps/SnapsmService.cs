...
namespace GameSvr
{
    ...
    {
        private const int MaxMirrorConnections = 10;
        ...
        private SnapsmService()
        {
            ...
            _msgServer = new ISocketServer(MaxMirrorConnections, 512);
        ...
        private void MsgServerClientRead(object sender, AsyncUserToken e)
        {
            for (var i = 0; i < m_SrvArray.Length; i++)
            {
                ...
                {
                    lock (m_SrvArray[i].SocDataLock) { m_SrvArray[i].SocData = m_SrvArray[i].SocData + HUtil32.GetString(e.ReceiveBuffer, e.Offset, e.BytesReceived); }
                }
            }
        }
    ...
    {
        public readonly object SocDataLock = new object();
