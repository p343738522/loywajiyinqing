namespace GameSvr
{
    ...
    //   +0x0C8(50) NativeMakePosion 0x66D1D8 = `C2 04 00`（ret 4，空函数）← 本类落地。
    ...
    // ── fail-closed：除 +0xC8 外全为属性/空虚槽（C# 侧非虚具体函数，无可覆写入口）或依赖未命名父级字段。
    //    +0xC8 原卡点已闭合：NativeMakePosion 现为虚方法（AttackIceTower 同槽）。无抗性 RNG；
    //    MagGroupAmyounsul 48 仍关。NativeParalysisResistPercent 仍 0。
    //   语义：“道具攻击怪”(掉落/受击特化)，类差异主要在 Monsters.DB 数据。
    ...
    public class ItemAttMon : Monster
    {
        ...
        // 战神 VMT+0xC8 = sub_66D1D8 `C2 04 00`：空函数，ret 4。
