...
namespace GameSvr
{
    public class BoneKingMonster : Monster
    {
        ...
        private void CallSlave()
        {
            ...
            for (var i = 0; i <= nC; i++)
            {
                ...
                BaseObject = M2Share.UserEngine.RegenMonsterByName(m_PEnvir, n10, n14, sMonName[M2Share.RandomNumber.Random(3)]);
                if (BaseObject != null)
                {
        ...
        public override void Run()
        {
            ...
            if (!m_boGhost && !m_boDeath && m_wStatusTimeArr[Grobal2.POISON_STONE] == 0 && HUtil32.GetTickCount() - m_dwWalkTick >= m_nWalkSpeed)
            {
                if ((HUtil32.GetTickCount() - m_dwSearchEnemyTick) > 8000 || (HUtil32.GetTickCount() - m_dwSearchEnemyTick) > 1000 && m_TargetCret == null)
                {
                    ...
                    SearchTarget();
                    if (m_nDangerLevel > (double)m_WAbil.HP / m_WAbil.MaxHP * 5 && m_nDangerLevel > 0)
                    {
                        m_nDangerLevel -= 1;
                        CallSlave();
                    }
