...
namespace GameSvr.Configs
{
    public class GameCmdConfig : IniFile
    {
        ...
        public void LoadConfig()
        {
            ...
            // 读取自定义命令别名 [CustomAlias]
            // 格式: 原始命令名=自定义别名（支持中文、字母、数字）
            // 例: AddGameGold=充值金币
            foreach (var key in GetSectionItemName("CustomAlias"))
            {
                var alias = ReadString("CustomAlias", key, "");
                if (!string.IsNullOrWhiteSpace(alias))
                    M2Share.CommandSystem.RegisterCommand(key, alias.Trim());
            }
        }

        /// <summary>
        /// 直接从磁盘重新读取 [CustomAlias] 节，绕过 IniFile 内存缓存，
        /// 用于运行时热重载别名，无需重启服务器。
        /// </summary>
        public void ReloadCustomAlias()
        {
            try
            {
                var lines = System.IO.File.ReadAllLines(FileName, HUtil32.GbkEncoding);
                var inSection = false;
                foreach (var rawLine in lines)
                {
                    var line = rawLine.Trim();
                    if (line.Equals("[CustomAlias]", StringComparison.OrdinalIgnoreCase))
                    ...
                    if (!string.IsNullOrWhiteSpace(key) && !string.IsNullOrWhiteSpace(val))
                        M2Share.CommandSystem.RegisterCommand(key, val);
                }
            }
            catch { }
        }
